    ...
    if (api_key.empty())
    {
        ...
        if (content.find("API-KEY:") == 0)
        {
            ...
            boost::asio::async_write(downstream_socket_,
                                     response_,
                                     boost::bind(&bridge::handle_upstream_write, // 继续从下游读取
                                                 shared_from_this(),
                                                 boost::asio::placeholders::error));
        }
        else
...
void bridge::create_response_(const std::string &error_message)
{
    std::ostream response_stream(&response_);
    // 使用write避免整数被转换成ASCII字符串,明确写入4字节长度
    // 处理总长度
    uint32_t total_len = htonl(error_message.length() + 8);
    response_stream.write(
        reinterpret_cast<const char *>(&total_len),
        sizeof(total_len));

    // 处理内容
    uint32_t error_message_len = htonl(error_message.length());
    response_stream.write(
        reinterpret_cast<const char *>(&error_message_len),
        sizeof(error_message_len));
