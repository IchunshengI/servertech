...
## 6. Protobuf vs Json序列化组件
protobuf 和 json均为序列化组件，此节补充protobuf的技术优势


我们这里做一个简单的测试，首先是小数据还原的场景，对应的protobuf文件为
```c++
...
```
这里呢我们将这个PersonData序列化和反序列+还原对象，去测试这种情况下的执行速度以及内存大小 (boost::json库)

| 类型                     | 序列化 (protobuf / json) | 反序列化 (protobuf / json) | 类还原 (protobuf / json) | 内存大小 (protobuf / json) | 压缩率 (protobuf / json) | 性能提升（以 JSON 为基准） |
|--------------------------|---------------------------|-----------------------------|---------------------------|-----------------------------|---------------------------|-----------------------------|
| 多成员，小内存           | 19 us / 3 us              | 36 us / 13 us               | 27 us / 66 us             | 1646 B / 3448 B             | 47.7% / 100%              | 0%                          |
| 多成员，大内存           | 92 us / 16 us             | 182 us / 66 us              | 119 us / 327 us           | 8580 B / 17448 B            | 49.2% / 100%              | +3.91%                      |
| 单字段，短字符串 (4kB)   | 936 ns / 602 ns           | 1139 ns / 463 ns            | 212 ns / 857 ns           | 4114 B / 4131 B             | 99.6% / 100%              | −18.9%                      |
| 单字段，长字符串 (1MB)   | 197 us / 572 us           | 201 us / 68 us              | 30 us / 31 us             | 1048595 B / 1048611 B       | 99.9985% / 100%           | +36.2%                      |

可以看出protobuf在小对象的情况下主要是压缩率较高，能够减轻网络传输负载。在具有长字符串的情况下能够拥有较快的序列化速度。
